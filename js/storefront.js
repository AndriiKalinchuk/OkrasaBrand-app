...
async function shopifyStorefrontQuery(query, variables) {
  const res = await fetch(
    `https://${SHOPIFY_API_DOMAIN}/api/2023-01/graphql.json`,
    {
      method: "POST",
...
async function checkoutCreate() {
  const res = await shopifyStorefrontQuery(SHOPIFY_QUERIES.checkoutCreate, {
    input: {},
  });
...
async function checkoutFetch() {
  ...
  const res = await shopifyStorefrontQuery(SHOPIFY_QUERIES.checkoutFetch, {
    id: id,
  });
...
async function checkoutAdd(items) {
  ...
  const res = await shopifyStorefrontQuery(
    SHOPIFY_QUERIES.checkoutLineItemsAdd,
    { id: id, lineItems: revisedItems }
  );
...
async function checkoutUpdate(lineItems) {
  // array of {id: 'LineItemID', quantity: int, customAttributes: []}
  const id = (await checkoutFetch()).id;
  const res = await shopifyStorefrontQuery(
    SHOPIFY_QUERIES.checkoutLineItemsUpdate,
    { id: id, lineItems: lineItems }
  );
...
async function checkoutRemove(lineIDs) {
  const id = (await checkoutFetch()).id;
  const res = await shopifyStorefrontQuery(
    SHOPIFY_QUERIES.checkoutLineItemsRemove,
    { id: id, lineItemIds: lineIDs }
  );
...
async function checkoutUpdateNote(note) {
  const id = (await checkoutFetch()).id;
  const res = await shopifyStorefrontQuery(
    SHOPIFY_QUERIES.checkoutAttributesUpdateV2,
    {
      id: id,
      input: {
        note: note,
